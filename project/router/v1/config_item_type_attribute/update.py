...
from project.schema.v1.config_item_type_attribute.update import UpdateConfigItemTypeAttributeParams
...
class ConfigItemTypeAttribute(Endpoint):
    ...
    @webserver.api('/apis/v1/ci-type-attribute/<int:ci_type_attribute_id>/update', methods=['PATCH'], tags=['ci_type_attribute'])
    def update(
            self, service: Service, request: Request,
            ci_type_attribute_id: int = params.Path(Required, description='配置项类型属性ID'),
            body: UpdateConfigItemTypeAttributeParams = params.Body(Required, description='请求体')
    ) -> None:
        """ 更新配置项类型属性
        ...
        ci_type_attribute_id = request.path_group_dict['ci_type_attribute_id']
        params = UpdateConfigItemTypeAttributeParams(**data)
        ...
        with safe_transaction(service.orm, commit=True) as session:
            ...
            ci_type_attribute_manager.update_by_id(ci_type_attribute_id, validated_data=validated_data)
