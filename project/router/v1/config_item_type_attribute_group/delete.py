...
class ConfigItemTypeAttributeGroup(Endpoint):
    ...
    @webserver.api('/apis/v1/ci-type-attribute-group/<int:ci_type_attribute_group_id>/delete', methods=['DELETE'], tags=['ci_type_attribute_group'])
    def delete(
        ...
        @param service: 服务对象
        @param request: 请求对象
        @param ci_type_attribute_group_id: 配置项类型属性分组
        @return: None
        """
        ...
        with safe_transaction(service.orm, commit=True) as session:
            ...
            ci_type_attribute_group_manager.delete_by_id(ci_type_attribute_group_id)
